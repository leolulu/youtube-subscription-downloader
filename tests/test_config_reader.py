...
class TestLoadConfig:
    def test_file_not_exists_creates_default_and_exit(self, tmp_path, caplog):
        """测试文件不存在时创建默认TOML并退出。"""
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            content = f.read()
        assert "query_limit = 10" in content
        assert 'proxy = "socks5://127.0.0.1:10808"' in content

        # 检查输出
        assert "已创建默认配置文件" in caplog.text
        assert "请编辑 config.toml 文件" in caplog.text

    def test_missing_keys_exits_with_message(self, tmp_path, caplog):
        """测试缺少必需键时退出并提示。"""
        file_path = tmp_path / "config.toml"
        content = """
interval_min = 30
...
"""
        ...
        assert "download_dir" in caplog.text

    def test_invalid_type_exits(self, tmp_path, caplog):
        """测试类型无效时退出。"""
        file_path = tmp_path / "config.toml"
        content = """
...
"""
    ...
    def test_toml_parse_error_exits(self, tmp_path, caplog):
        """测试TOML解析错误时退出。"""
        file_path = tmp_path / "config.toml"
